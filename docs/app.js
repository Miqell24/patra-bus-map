...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: Αστικό ΚΤΕΛ Αχαΐας Α.Ε. (patra.citybus.gr)' }));
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · Timetables: Αστικό ΚΤΕΛ Αχαΐας Α.Ε. (patra.citybus.gr)';
      ...
      a.download = `patra-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `patra-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // knot. Open on the city centre, the ring road in the frame; the VIPE estate 16 km
  // out on the Sykourio road is one pan away.
  map.jumpTo({ center: [21.7450, 38.2500], zoom: 12.4 });
}
