...
// Patra: ONE feed, and this map builds it itself — see pipeline/citybus-feed.mjs.
// 30 bus lines of Αστικό ΚΤΕΛ Αχαΐας in the family's navy. No trolleybus, no tram,
...
// ONE feed, ONE cfg.
//
// Larisa publishes no GTFS anywhere — the operator's site has timetables and
...
// of the operator's own passenger site (larisa.citybus.gr → rest.citybus.gr), which
// carries every pole, every line, every pattern and a polyline for each.
//
// allVariants: that API publishes PATTERNS, not runs — 75 patterns across 18 lines,
...
const SEED = {
  ...
  // Patra (11.09.2026): the operator writes many names in lower case without
  // accents; common words only — surnames stay as the operator spells them
  ΟΜΟΝΟΙΑ: 'Ομόνοια', ΑΝΘΟΠΩΛΕΙΟ: 'Ανθοπωλείο', ΣΤΡΑΤΙΩΤΙΚΟ: 'Στρατιωτικό', ΥΠΕΡΑΣΤΙΚΟ: 'Υπεραστικό',
  ΣΙΔΗΡΟΔΡΟΜΙΚΕΣ: 'Σιδηροδρομικές', ΥΠΟΔΟΧΗΣ: 'Υποδοχής', ΜΕΛΙΝΑ: 'Μελίνα', ΤΡΟΧΑΙΑΣ: 'Τροχαίας', ΑΡΧΑΙΟ: 'Αρχαίο',
  ΠΛΑΣΤΙΚΩΝ: 'Πλαστικών', ΦΑΝΑΡΙ: 'Φανάρι', ΠΟΤΑΜΙ: 'Ποτάμι', ΓΕΦΥΡΑΚΙ: 'Γεφυράκι', ΣΤΡΟΦΕΣ: 'Στροφές', ΕΞΕΔΡΑ: 'Εξέδρα',
  ΨΗΣΤΑΡΙΑ: 'Ψησταριά', ΜΑΓΑΖΙΑ: 'Μαγαζιά', ΒΥΡΣΟΔΕΨΕΙΟ: 'Βυρσοδεψείο', ΠΤΩΧΟΚΟΜΕΙΟ: 'Πτωχοκομείο', ΦΥΛΑΚΙΟ: 'Φυλάκιο',
  ΠΑΙΔΟΤΟΠΟΣ: 'Παιδότοπος', ΣΠΥΡΙΔΩΝΑΣ: 'Σπυρίδωνας', ΜΗΝΑΣ: 'Μηνάς', ΨΗΛΑ: 'Ψηλά', ΑΠΟΛΑΥΣΗ: 'Απόλαυση', ΜΕΡΙΜΝΑ: 'Μέριμνα',
  ΠΕΥΚΟ: 'Πεύκο', ΑΡΤΑΣ: 'Άρτας', ΟΛΥΜΠΙΟΝ: 'Ολύμπιον', ΕΥΡΙΒΙΑΔΟΥ: 'Ευρυβιάδου', ΘΟΥΚΙΔΙΔΟΥ: 'Θουκυδίδου',
  ΚΑΡΠΕΝΗΣΙΩΤΟΥ: 'Καρπενησιώτου', ΤΑΜΠΑΧΑΝΩΝ: 'Ταμπαχανών', ΜΩΡΕΑΣ: 'Μωρέας', ΠΑΡΚΙΝΚ: 'Πάρκινγκ', ΤΣΑΟΥΣΗ: 'Τσαούση',
  ΣΠΗΛΙΩΤΑΚΟΠΟΥΛΟΥ: 'Σπηλιωτακοπούλου', ΑΧΑΪΑΣ: 'Αχαΐας', ΡΟΪΤΙΚΑ: 'Ροΐτικα',
};
...
const greekName = (n) => {
  // Patra writes the dialytika as apostrophes: ΑΧΑ'Ι'ΑΣ, ΡΟ'Ι'ΤΙΚΑ
  let s = String(n || '').replace(/'Ι'/g, 'Ϊ').replace(/'Υ'/g, 'Ϋ').replace(/\s+/g, ' ').trim()
...
const MODES = [{
  ...
  // Giannouli, Falani and Dasochori north, Terpsithea south-west, Nikaia
  // south, Omorfochori east, the VIPE estate 16 km out — 2 × 2 tiles
  ...
  feeds: [
    { tag: 'la', dir: 'data/gtfs', routeTypes: ['3'], allVariants: true,
