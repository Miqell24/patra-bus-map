#!/usr/bin/env bash
# Downloads input data: the Larisa GTFS this project writes itself, the OSM
# extract, MapLibre GL. Everything is cached — re-running only fetches what is
# missing.
#
# There is NO Larisa GTFS to download. The operator publishes timetables on
...
# of the operator's own passenger site (patra.citybus.gr → rest.citybus.gr).
set -euo pipefail
cd "$(dirname "$0")/.."
...
if [ ! -f data/gtfs/routes.txt ]; then
  echo "== patra.citybus.gr API -> data/gtfs =="
